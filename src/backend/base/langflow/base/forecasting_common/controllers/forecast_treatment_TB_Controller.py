...
class ForecastTreatmentTBController():
    ...
    def calc_treatment_pat_forecast(# self variables passed in
        ...
        for i in range(num_months_treatment):
            ...
            # PATIENT LEAVING TREATMENT BY TREATMENT MONTH
            ...
            if(i == 0):
                ...
                updated_meta_data = updated_meta_data.add_col_meta_data(frame = updated_meta_data,
                                                                        ...
                                                                        display_name = f"# of patients in '{display_name}' treatment month {i+1}",
                ...
                # that we calculated above (updated_data OR updated_meta_data) from the input patient flow (last id of updated_data or update_meta_data).
                # this represents the number of diagnosed patients who get to the end of the first month.

                # In the DATA calculations:
                # PM = updated_data[incoming_patient_flow]
                # CM = updated_data[curr_on_treat_col_id]
                updated_data[curr_leaving_treat_col_id] = updated_data[incoming_patient_flow] - updated_data[curr_on_treat_col_id]
                ...
                # PM = updated_meta_data.get_last_id()
                ...
                updated_meta_data = updated_meta_data.add_col_meta_data(frame = updated_meta_data,
                                                                        ...
                                                                        data_type = ForecastDataSeriesMetaDataDataType.FLOAT,
                                                                        display_type = ForecastDataSeriesMetaDataDataType.INT,
                                                                        display_name = f"# of patients leaving '{display_name}' in treatment month {i+1}",
                                                                        data_values = updated_data[curr_leaving_treat_col_id].to_list(),
                                                                        validation = [{ForecastDataSeriesMetaDataValidationSchema.INPUT_RESTRICTION: ForecastDataSeriesMetaDataValidateInputRestrictions.READ_ONLY}],
                                                                        pred = preds_leaving,
                                                                        verify_integrity=True,
                                                                        drop_dups = False)
            ...
            # ==========================
                ...
                # first month (CM) (i.e. updated_data[curr_on_treat_col_id]), that we just calculated from the number of patients in the month BEFORE THE 
                ...
                # CM = updated_data.loc[0, curr_on_treat_col_id]
                updated_data.loc[0, curr_leaving_treat_col_id] = pre_forecast_patient_flow_data.loc[0, pfpf_col_id] - updated_data.loc[0, curr_on_treat_col_id]
                ...
                # (i.e. updated_data[curr_on_treat_col_id], same as in part one), from the number of patients at the end of the prevous month (PM)
                # (i.e. updated_data[prev_on_treat_col_id] shifted by 1 month prior) for the same MONTH OF TREATMENT

                # In the DATA calculations:
                # PM = updated_data[prev_on_treat_col_id].shift(1)[1:].reset_index(drop=True)
                # CM = updated_data.loc[1:, curr_on_treat_col_id].reset_index(drop=True)
                updated_data.loc[1:, curr_leaving_treat_col_id] = (updated_data[prev_on_treat_col_id].shift(1)[1:].reset_index(drop=True) - updated_data.loc[1:, curr_on_treat_col_id].reset_index(drop=True)).to_list()
        ...
        # Need to split this into two DataFrames and ForecastMetaDataFrame as the last column of each will be the total patients on therapy per month
        # or the total patients leaving therapy per month
        pat_by_treatment_month_data = copy.deepcopy(updated_data)
        pat_by_treatment_month_meta_data = copy.deepcopy(updated_meta_data)
                    
        pat_leaving_by_treatment_month_data = copy.deepcopy(updated_data)
        ...
        # generate a totals PATIENTS ON TREATMENT column
