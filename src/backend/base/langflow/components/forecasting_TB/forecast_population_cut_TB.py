...
class ForecastPopulationCutTB(Component):

    # CONSTANTS
    # =========
    MAX_SEGMENTS = 100
    SEGMENT_COL_PREFIX = "popcut_"

    # COMPONENT META-DATA
    # -------------------
    display_name: str = "Population Cuts TB"
    description: str = "Apply a timescale specific % decrease critera from the population flow input."
    icon = "Scissors"
    name: str = "PopulationCutTB"
    ...
    inputs = [
        ...
        TableInput(
            name="segment_table",
            display_name="Population cut",
            info="The percentages of total patient flow cut at each time period",
            required=True,
            show=True,
            dynamic=True,
            real_time_refresh=True,
            table_schema=[
                ...
                },
                {
                    ...
                    "type": "float",
                    "description": "The % of the incoming population which is retained through.",
                    "edit_mode": EditMode.INLINE,
                    "disable_edit": False,
                },
            ],
    ...
    # -----------------
    outputs = [
        Output(display_name="Retained Patient Flow", name="retained_patient_model", method="update_forecast_model_retained"),
        Output(display_name="Cut Patient Flow", name="cut_patient_model", method="update_forecast_model_cut", hidden=True),
    ]
    ...
    def update_forecast_model_retained(self, seg_num=1) -> DataFrame:
        ...
        # get segment name
        seg_name = f"{ForecastPopulationCutTB.SEGMENT_COL_PREFIX}{self._id}"
    ...
    def generate_table_values(self, field_value: str, field_name: str) -> List[dict]:
        ...
        if(not hasOldValues):
            ...
            for curr_row in segment_table:
                for i in range(num_segments):
                    curr_row[f"{ForecastPopulationCutTB.SEGMENT_COL_PREFIX}{i+1}"] = ForecastDataModel.EDITABLE_VALUES_TOKEN
            return(segment_table)
        ...
        # otherwise, resize the exist values into the new size (note: always add the dates in)
            ...
            new_df = ForecastFormModelUtilities.refill_drataframe(new_dim_rows=num_rows, new_dim_cols=num_cols, prev_data=old_values_df, col_name_prefix=ForecastPopulationCutTB.SEGMENT_COL_PREFIX, dates=dates)
